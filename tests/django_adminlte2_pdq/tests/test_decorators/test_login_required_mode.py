...
class LoginModeMixin:
    ...
    def test__login_required_decorator(self):
        ...
        # Invalid decorator used for Login Required mode. Should raise error for all user types.
        for user_instance, user_str in self.user_list__full:
            with self.subTest(f"As {user_str}"):
...
class TestLoginRequiredAuthenticationDecoratorsWithLoginWhitelist(BaseDecoratorTestCase, LoginModeMixin):
    ...
    * "one_of_perms"/"full_perms" - Raises an "ineffective whitelist" warning.
    ...
    def test__no_decorators(self):
        ...
        # The "anonymous" and "inactive" users should now be able to access the page.
        # Should succeed and load as expected for all users.
        for user_instance, user_str in self.user_list__full:
            with self.subTest(f"As {user_str}"):
