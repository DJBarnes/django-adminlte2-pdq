...
can access the view that uses this decorator. Django's default login_required
decorator is used behind the scenes and is only recreated in this package in
order to add the magic of automatic sidebar node rendering.
...
.. warning::
    ...
    We wanted to prevent the Sample2 route from being accessed by people that
    ...
    route. This is the "loose" part of the loose policy as it default to
    everyone being able to see every route unless a permission is explicitly
    set on that route to add security.
